...
function processSum(numberList, cb) {
  const sum = numberList.reduce((acc, currentVal) => acc + currentVal, 0)
  return cb(sum)
}
...
function processContains(item, list, cb) {
  const container = list.filter(contains => contains === item);
...
function processDuplicateFree(list, cb) {
  let deDuped = cb(list.filter((item, index) => list.indexOf(item) === index));
  return deDuped;
}
